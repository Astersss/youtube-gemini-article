...
1. **拆分 teaser**：把 Introduction 章节里嘉宾的连续独白按话题切换点（语气/指代/主题变化）拆成 N 个独立 teaser，每个 teaser 一个简短 ###。
   · 严禁把多个 teaser 合并成 1 个 ### 或塞进同一段回答里。

2. **为每个 teaser 配齐 Q&A**：每个 teaser ### **必须**写出完整 **[提问者名]:** + **[嘉宾名]:** 一问一答结构。根据 teaser 本身的形态分三种来源：
   ...
   · 严禁：teaser ### 下只放嘉宾发言、缺提问段。
   · 每个 teaser 回答**保持简短**（1-2 段），不要硬凑长度。
...
4. **开头 ## 的标题**：
   · **根据当前视频的 teaser 群实际内容自创**，必须有内容感、能透露 teaser 群的共同议题。
